...
num=`mysql   ${mysql_conn}   -e "use ${databasename};select id,nickname from tbl_player where nickname in (select nickname from tbl_player where nickname <> ''  group by nickname  HAVING count(nickname)>=2);"| wc -l`
if [[ ${num} == 0 ]]
then
	echo -e "\033[32m${databasename}询玩家重复名数量为0，不需要修改\033[0m"
	exit 1
else
	
	echo -e "\033[32m${databasename}询玩家重复名数量为${num}，需要修改\033[0m"
#	confirm
